...
async function handleRequest(request, env, ctx) {
  ...
  if (path === '/api/pok/create-payment' && method === 'POST') {
    try {
      ...
      const orderPayload = {
        ...
        redirectUrl: frontend + '/partner-dashboard?payment=success',
        failRedirectUrl: frontend + '/partner-dashboard?payment=cancel',
  ...
  if (path === '/api/pok/webhook' && method === 'POST') {
    try {
      ...
      const paid = ['paid', 'captured', 'completed', 'complete', 'success', 'succeeded', 'capture', 'confirmed'].some(s => statusStr.includes(s));
      if (!paid) {
